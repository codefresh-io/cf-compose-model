...
const ParsingError                 = require('./components/validations/ParsingError');
...
const Warning                      = components.WarningsComponents;
...
const NotSupportedByPolicyWarnings = require('./components/validations/FieldNotSupportedByPolicy');
...
class CFComposeModel {
            ...
            .then(() => {
                ...
                if (!accessibility.isGlobalVolumeAllowed()) {
                    return Promise.map(volumeKeys, volumeName => {
                        ...
                        const warning = new NotSupportedByPolicyWarnings(volumeName, volumes[volumeName],'Avoid using global volumes', 'Warning at: volumes.redis-data', this.getServiceByName(volume.getName()), false, true);
                        warning.readable =
    ...
    static parse(yaml) {
        if (_.isString(yaml)) {
            try {
                ...
                return Promise.resolve(new ParsingError(yaml, err.message).format());
            }
        }
    ...
    static getParserForYaml(yaml) {
        ...
        if (!yaml) {
            return new ErrorComponent('YAML_NOT_SUPPLIED', yaml, 'yaml file cannot be parssed');
            ...
            return new ErrorComponent('NOT_SUPPORTED_YAML', yaml);
