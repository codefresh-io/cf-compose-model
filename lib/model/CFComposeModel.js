...
class CFComposeModel {
    ...
    getWarnings() {
        ...
        if(_.get(policy, 'volumes.supported', false)){
            const warning = new Warning('GLOBAL_VOLUMES_NOT_SUPPORTED', this.getAllVolumes(), 'Avoid using global volumes', `Warning at: volumes`);
            res.push(warning);
        }

        // if (Object.keys(this.getAllVolumes()).length) {
        //     _.forOwn(_.get(this, 'policy.volumes.global', []), (violation) => {
        //         const warning = this._createWarning(violation, 'volumes', this.getAllVolumes());
        //         if (warning) {
