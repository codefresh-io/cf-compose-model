...
class CFComposeModel {
                ...
                if (!accessibility.isGlobalVolumeAllowed()) {
                    return Promise.map(volumeKeys, volumeName => {
                        ...
                        if(volume.isExternalVolume() && isExternalVolumesAllowed) {
                            ...
                            warning.readable = `${warning.getMessage()}. The value ${warning.getData()} is not supported. ${warning.getSuggestion()}`;
                            res.push(warning);
                            return;
                        }
    ...
    static getParserForYaml(yamlObj) {
        const parsers = CFComposeModel.getParserForYaml(yamlObj);
        if(parsers.Parser){
    ...
    static setDefaultPromise(Promise){
      config.setDefaultPromise(Promise);
    }
