...
/**
 * CFComposeModel is a model that can be used the to manged all types of compositions.
 * The model can be initiate by one of the parses given an input or from scratch.
 */
...
class CFComposeModel {
    ...
    _mapOver(field, cb) {
        ...
        const p = Promise.map(keys, (key) => {
            const value = object[key];
            return cb(key, value);
        });
        return promiseProvider.resolve(p);
    }
    ...
    allowPrivilegedMode(){
        this.accessiblity.services.allowPrivilegedMode();
    }

    disallowPrivilegedMode(){
        this.accessiblity.services.disallowPrivilegedMode();
    }
    ...
    /**
     * @param user - *nix uid (for example, '0' - is for root)
     * @param overwrite - defines whether to overwrite property value
     */
    setUserForServices(user, overwrite = false) {
        _.chain(this.getAllServices())
            .values()
            .filter(service => overwrite || !service.getByName(USER_FIELD))
            .forEach(service => service.setAdditionalData(USER_FIELD, user))
            .value();
    }

    _parseService(name, obj){
    ...
    parseService(name, obj) {
        return promiseProvider.resolve(this._parseService(name, obj));
    }
     ...
     */
    getImageNames() {
        const p = Promise.map(this.getServicesOrder(), (name) => {
    ...
    static parse(yaml, policy) {
        ...
        const p = Parser.parse(yaml, policy);
        const onReject = err => {
    ...
    /**
     * Format and strinfigy warnings
     * @param [warnings]
     */
    static stringifyWarnings(warnings){
        const p = Promise.map(warnings, warning => {
    ...
    /**
     * Format and strinfigy errors
     * @param [errors]
     */
    static stringifyErrors(errors){
        const p = Promise.map(errors, error => {
            return `${JSON.stringify(error.format())}\n`;
        });
        return promiseProvider.resolve(p);
    }

    static setPromiseProvider(Promise){
        promiseProvider.setProvider(Promise);
    }
}
