...
class Port extends CFLeaf {
    constructor(portBuilder) {
        ...
        super(portBuilder.parentFieldName);
        this.source      = portBuilder.source;
        this.target      = portBuilder.target;
        this.protocol    = portBuilder.protocol;
        this.stringValue = `${this.source ? this.source + ':' : ''}${this.target}${this.protocol ?
        '/' + this.protocol : ''}`;
    }
