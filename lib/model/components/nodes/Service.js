                ...
                if (_.isArray(field)) {
                    _.forEach(field, (value) => {
                        if (value instanceof Leaf) {
                            const warningsArr = value.getWarnings(policy[value.parentFieldName]);
                    ...
                    if (field instanceof Leaf) {
                        const warningsArr = field.getWarnings(policy[field.parentFieldName]);
                        _.forEach(warningsArr, (warning) => {
                            warning.message = `Warning: at service ${this.name}.${fieldName}`;
    ...
    actOnWarning(violation) {
        const cases = {
            'NOT_SUPPORTED': {
                ...
                'build': () => {
                    delete this[violation.path];
                    this.image = new Image(undefined, this.name, 'latest', 'image');
                }
            }
