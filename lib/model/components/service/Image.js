...
class Image extends Base {
    ...
    getWarnings(policy){
        ...
        if(!this.getTag() && policy.isMissingTagReproduceWarning()){
            const warning   = new Warning('MISSING_IMAGE_DETAILS_TAG', `${this.getOwner() ? this.getOwner() + ':' :
                ''}${this.getRepo()}`, 'latest');
            warning.autoFix = true;
        ...
        if(!this.getOwner() && policy.isMissingOwnerReproduceWarning()){
            const warning              = new Warning('MISSING_IMAGE_DETAILS_OWNER_NAME', `${this.getRepo()}${this.getTag() ?
            ':' + this.getTag() : ''}`, '');
            this.warnings.push(warning);
        }
