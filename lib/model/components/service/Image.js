...
class Image extends Base {
    ...
    getWarnings(policy) {
        ...
        if (!this.getTag() && policy.isMissingTagReproduceWarning()) {
            ...
            this.getOwner() + ':' :
                ''}${this.getRepo()}`;
            const warning   = new FieldNotSupportedByPolicy(this.getName(), this.getImageStandardName(), suggestion, 'Add tag', true);
        ...
        if (!this.getOwner() && policy.isMissingOwnerReproduceWarning()) {
            ...
            ':' + this.getTag() : ''}`;
            const warning = new FieldNotSupportedByPolicy(this.getName(), this.getImageStandardName(), suggestion, '');
            this.warnings.push(warning);
        }
