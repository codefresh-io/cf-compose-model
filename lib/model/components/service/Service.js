...
class Service extends Base {
        ...
        return Promise.resolve()
            .then(() => {
                if (this['container_name']) {
                    const warning   = new Warning('CONTAINER_NAME_NOT_SUPPORTED', 'container_name', 'Avoid using container_name field', `Warning: at service ${this.getName()}.container_name`);
                    warning.readable = `${warning.getMessage()}. The key ${warning.getActualData()} is not supported. ${warning.getSuggestion()}`;
                    ...
                    return Promise.map(ports, (port) => {
                        return port.getWarnings()
                            .map(warning => {
                                ...
                                warning.readable = `${warning.getMessage()}. The value ${warning.getActualData()} is not allowed. ${warning.getSuggestion()}`;
                                return warning;
                            });
                    })
                        .then(warnings => {
    ...
    getErrors() {
        ...
        const image = this.getImage();
        if (image instanceof ErrorComponent) {
            res.push(image.formatError());
        }

        const ports = this.getPorts();
        return Promise.map(ports || [], (port) => {
            if (port instanceof ErrorComponent) {
                res.push(port.formatError());
            }
            return;
        })
            .then(() => {
            ...
            .then(() => {
                ...
                return Promise.map(fields, (key) => {
                    if(this[key] instanceof ErrorComponent){
                        res.push(this[key].formatError());
                    }
                });
            })
            .then(() => {
    ...
    getOrder(){
        ...
        if(!order.length){
            return _.keys(_.omit(this,['_name', 'warnings', '_order', '_isPortFixed']));
        }
        return order;
    }
    ...
    mapOverAdditionalData(cb){
        const almostAllFields = this._getAllAdditionalData();
        delete almostAllFields['environment'];
        delete almostAllFields['labels'];
