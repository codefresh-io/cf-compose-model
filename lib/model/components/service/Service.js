...
const InvalidSyntexByParser             = require('./../validations/InvalidSyntexForParser');
const FieldNotSupportedByOriginalParser = require('./../validations/FieldNotSupportedByOriginalParser');
const FieldNotSupportedByPolicy         = require('./../validations/FieldNotSupportedByPolicy');
...
class Service extends Base {
    ...
    _getAllAdditionalData() {
        return _.omit(this, ['image', 'ports', 'volumes', '_metadata', '_name', 'warnings']);
    }
        ...
        return Promise.resolve()
            .then(() => {
                if (this['container_name']) {
                    const warning = new FieldNotSupportedByPolicy('container_name', this['container_name'], 'Field not supported , avoid using "container_name"', `Warning: at service ${this.getName()}.container_name`, true);
                    warning.readable =
                        `${warning.getMessage()}. The key ${warning.getData()} is not supported. ${warning.getSuggestion()}`;
                    warning.setAutoFix();
            ...
            .then(() => {
                if (this['build'] && !policy.isBuildSupported()) {
                    const warning = new FieldNotSupportedByPolicy('build', this['build'], `image: ${this.getName()}`, `Warning: at service ${this.getName()}.build. Try replace with image instead`, true, false);
                    warning.readable =
                        `${warning.getMessage()}. The key ${warning.getData()} is not supported. ${warning.getSuggestion()}`;
                    warning.setAutoFix();
                    this.warnings.push(warning);
    ...
    fixWarnings(onlyAutoFix) {
        this.setPortBeenFixed(); //refactor
    ...
    getErrors() {
        ...
        const image = this.getImage();
        if (image instanceof InvalidSyntexByParser) {
            res.push(image.format());
        }

        const ports = this.getPorts();
        return Promise.map(ports || [], (port) => {
            if (port instanceof InvalidSyntexByParser) {
                res.push(port.format());
            }
            return;
        })
            .then(() => {
    ...
    getOrder() {
        ...
        if (!order.length) {
            return _.keys(_.omit(this,
                ['_name', 'warnings', '_order', '_isPortFixed', '_metadata']));
        }
        return order;
    }
    ...
    mapOverAdditionalData(cb) {
        const almostAllFields = _.omit(this, ['_metadata', '_name', 'warnings', '_order']);
        const keys            = _.keys(almostAllFields);
        return Promise.map(keys, (key) => {
            return cb(key, almostAllFields[key]);
        });
    }
