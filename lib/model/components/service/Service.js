...
class Service extends Base {
    ...
    _getWarnings(policy, { allowedVolumes, globalVolumeWithLocalDriver }){
        this.warnings = [];
            ...
            .then(() => {
                ...
                if (this.getVolumes() && !allowVolumeMapping) {
                    ...
                    return Promise.map(volumes, (volume) => {
                        return volume._getWarnings(allowVolumeMapping,
                            {
    ...
    getWarnings(policy, { allowedVolumes, globalVolumeWithLocalDriver }) {
        return promiseProvider.resolve(this._getWarnings(policy, { allowedVolumes, globalVolumeWithLocalDriver }));
    }
    ...
    addVolume(volume) {
        ...
        if (!this.isKeyExistInOrder('volumes')) {
            ...
            this.setVolumesOriginalType('Array');
        }
     ...
     * Will replace previous defined image if exist
    ...
    /**
     * Add new environment variable to the service
     * @param key {string}
     * @param value {string}
     * @return {*}
     */
    ...
    /**
     * Get additional fields if exist on the service
     * Will not return image, volumes or ports
     * @param fields {array}
     * @return {*}
     */
    get(fields) {
    ...
    /**
     * Return the environment variable value by the key
     * @param name
     * @return {*}
     */
    ...
    mapOverEnvironments(cb) {
        return promiseProvider.resolve(this._mapOverEnvironments(cb));
    }

    _mapOverAdditionalData(cb){
    ...
    mapOverAdditionalData(cb) {
        return promiseProvider.resolve(this._mapOverAdditionalData(cb));
    }
    ...
    mapOverLabels(cb) {
        return promiseProvider.resolve(this._mapOverLabels(cb));
    }
    ...
    mapOverVolumes(cb) {
        return promiseProvider.resolve(this._mapOverVolumes(cb));
    }

    _mergeWith(otherService){
    ...
    mergeWith(otherService) {
        return promiseProvider.resolve(this._mergeWith(otherService));
    ...
    mapOverExposedPorts(cb) {
        return promiseProvider.resolve(this._mapOverExposedPorts(cb));
    }
