...
class Service extends Base {
    ...
    mapOverEnvironments(cb) {
        let p;
        if (_.isArray(this.environment)) {
            p = Promise.map(this.environment, (env) => {
                const kv = env.split('=');
                return cb(kv[0], kv[1]);
            });
        } else {
            const keys = _.keys(this.environment);
            p          = Promise.map(keys, (key) => {
    ...
    mapOverVolumes(cb) {
        const volumes = this.getVolumes() || [];
        const p = Promise.map(volumes, cb);
        return promiseProvider.resolve(p);
    }
    ...
    mapOverExposedPorts(cb) {
        const p = Promise.map(this.getPorts(), (port) => {
