...
class NewServiceParser {
    ...
    _createWarningFieldNotSupportedByPolicy({ fieldName, fieldValue, serviceName, suggestion, autoFix, requireManuallyFix }) {
        const warning       = new FieldNotSupportedByPolicy(fieldName,
            fieldValue,
            suggestion,
            `Warning: at service ${serviceName}.${fieldName}.`,
            autoFix,
            requireManuallyFix);
        ...
        return Promise.map(serviceObjKeys, (fieldName) => {
            ...
            if (this._isSpecialField(fieldName)) {
                ...
                if (!this._isIsSupportedField(fieldName)) {
                    const err = new FieldNotSupportedByOriginalParser(fieldName,
                        fieldValue,
                        `Field '${fieldName}' is not supported by compose`);
    ...
    attemptToCreateImage(fieldName, imageString) {
        try {
            ...
            throw new InvalidSyntexForParser(fieldName,
                imageString,
                `Image must be string, got ${type(
                    imageString)}`);
        }
    }
    ...
    attemptToCreateVolume(fieldName, volumeString) {
        try {
            ...
            throw new InvalidSyntexForParser(fieldName,
                volumeString,
                `Volume must be string, got ${type(
