...
const NotSupportedField = require('./../components/validations/FieldNotSupportedByOriginalParser');
const InvalidSyntexForParser = require('./../components/validations/InvalidSyntexForParser');
            ...
            return Promise.map(keys, (fieldName) => {
                ...
                const cases      = {
                    'image': () => {
                        if (!_.isString(fieldValue)) {
                            const err = new InvalidSyntexForParser(fieldName, fieldValue, `Image must be string, got ${type(fieldValue)}`)
                ...
                if (cases[fieldName]) {
                    ...
                    if (_.indexOf(fields, fieldName) < 0) {
                        const err = new NotSupportedField(fieldName, fieldValue, `Field '${fieldName}' is not supported by compose v1`);
