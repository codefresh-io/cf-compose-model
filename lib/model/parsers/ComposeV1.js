    ...
    static parse(yaml) {
        ...
        _.forOwn(yaml, (serviceObj, name) => {
            ...
            _.forOwn(serviceObj, (fieldValue, fieldName) => {
                const cases = {
                    'image': () => {
                        leafs[fieldName] = components.Image.parse(fieldValue, fieldName);
                    },
                    'ports': () => {
                        const ports = leafs[fieldName] = [];
                        if (_.isArray(fieldValue)) {
                            _.forEach(fieldValue, (port) => {
                                const portInstance = components.Port.parse(port, fieldName);
                    ...
                    'volumes': () => {
                        ...
                        if (_.isArray(fieldValue)) {
                            _.forEach(fieldValue, (volume) => {
                                const volumeInstance = components.Volume.parse(volume, fieldName);
                                volumes.push(volumeInstance);
                            });
                        } else {
                            _.forOwn(fieldValue, (volume) => {
                                const volumeInstance = components.leafs.Volume.parse(volume,
                                    fieldName);
                                volumes.push(volumeInstance);
                            });
                        }
