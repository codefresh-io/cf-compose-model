...
describe('Compose v1 service parser', () => {
    ...
    tests.map(test => {
        it(test.title, () => {
            ...
            composeV1ServiceParser.defaultFieldValues = {};
    ...
    describe('Parsing with errors', () => {
        ...
        describe('privileged mode', () => {
            it('supported', () => {
                const serviceParser         = new ComposeV1ServiceParser('my-parser', 'my-value');
                const serviceInstance = new ServiceComponent('my-service');
                serviceParser.setAccessibility({
                    isPrivilegedModeSupported: () => true
                });
                serviceParser._actOnSpecialField(serviceInstance, 'privileged', true);
                expect(serviceParser.errors).to.have.length(0);
