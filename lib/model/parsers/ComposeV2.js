        ...
        return Promise.map(yamlServicesKeys, (serviceName) => {
            ...
            .then(() => {
                if (_.size(unsupportedKeys) > 0) {
                    ...
                    errors = errors.concat(keys.map(key => new InvalidSyntexForParser(key, unsupportedKeys[keys], `${key} is not supported by compose v2`)));
                }
                if (_.size(errors) > 0) {
                    const err        = new SyntaxError('FAILED_TO_PARSE');
                    err.errors       = errors;
                    err.originalYaml = YAML.dump(yaml);
