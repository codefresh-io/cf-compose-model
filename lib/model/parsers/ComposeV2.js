                    ...
                    if(networkObject) {
                        ...
                        return Promise.map(networkKeys, (fieldName) => {
                            ...
                            if (_.indexOf(fields.network, fieldName) < 0) {
                                const err = new ErrorComponent(fieldName, fieldValue, `Field '${fieldName}' is not supported by compose v2 under networks`);
                    ...
                    if(volumeObject) {
                        ...
                        return Promise.map(volumeKeys, (fieldName) => {
                            ...
                            if (_.indexOf(fields.volume, fieldName) < 0) {
                                const err = new ErrorComponent(fieldName, fieldValue, `Field '${fieldName}' is not supported by compose v2 under volumes`);
