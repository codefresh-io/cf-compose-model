        ...
        const composeV1FileNames = [{
            ...
            expectedWarnings: [
                ...
                },
                {
                    "actual": "build\n.",
                    "autoFix": true,
                    "message": "Warning: at service express-app-container.build. Try replace with image instead",
                    "name": "FIELD_NOT_SUPPORTED_IN_POLICY",
                    "requireManuallyFix": false,
                            ...
                            return compose.getWarnings()
                                .then(warnings => {
                                    _.forEach(warnings, (warning, index)=> {
                                        expect(warning.format()).to.be.deep.equal(value.expectedWarnings[index]);
                                    });
                                });
                        }
                    });
                            ...
                            .then(() => {
                                if (value.afterAutoFixCount) {
                                    ...
                                    return compose.getWarnings()
                                        .then((warnings) => {
                                                _.forEach(warnings, (warning, index) => {
                                                    expect(warning.format()).to.be.deep.equal(value.afterAutoFix[index]);
                                                });
                                        });
                                }
                            });
            ...
            return CFComposeModel.load(filePath)
                .then(compose => {
                    ...
                    return compose.getWarnings()
                        .then(warnings => {
                            _.forEach(warnings, (warning, index) => {
                                expect(warning.format()).to.be.deep.equal(expectedWarningArray[index]);
                            });
                        });
                });
        });
            ...
            return CFComposeModel.load(filePath)
                .then((compose) => {
                    return compose.getWarnings()
                        ...
                        .then(warnings => {
                            _.forEach(warnings, (warning, index) => {
                                expect(warning.format()).to.be.deep.equal(expectedWarrnigArrayAfterFix[index]);
                            });
                        });
                });
        });
...
`
    ...
    describe('Working with the model', () => {
        ...
        it('Should replace variables', () => {
            const filePath = `${__dirname}/ComposeV2/ex5.yaml`;
            return CFComposeModel.load(filePath)
                .then((compose) => {
