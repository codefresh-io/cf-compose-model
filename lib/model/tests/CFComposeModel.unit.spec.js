...
describe('CFComposeModel', () => {
    ...
    it.only(`Should fix all warnings and translate without calling getWarnings for file name`,
        () => {
            const filePath = path.resolve(__dirname, `./ComposeV1/ex3.yaml`);
            const compose  = CFComposeModel.load(filePath);
            compose.fixWarnings();
            expect(compose.translate()).to.be.deep.equal('');
        });
    ...
    describe('Compose v2', () => {
        it('Should find all warnings and fix possible', () => {
            ...
            const expectedWarningArray         = [
                ...
                {
                    "actual": "{\"redis-data\":{\"warnings\":[],\"name\":\"redis-data\",\"driver\":\"flocker\"}}",
                    "autoFix": false,
