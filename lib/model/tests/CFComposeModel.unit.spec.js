    ...
    describe('getParserForYaml', () => {
        ...
        it('Should return an error if the yaml cannot be parsed', () => {
            ...
            return CFComposeModel.load(filePath)
                ...
                .catch((err) => {
                    expect(err).to.be.deep.equal({"_name":"PARSING_FAILED","_message":"end of the stream or a document separator is expected at line 2, column 8:\n      image: ubuntu\n           ^","originalYaml":"app\n  image: ubuntu"});
                    return err.asyncToString()
                        .then(string => {
                           expect(string).to.be.equal(`Error: PARSING_FAILED
