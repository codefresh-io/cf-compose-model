...
`
    ...
    describe('getParsersForYaml', () => {
      ...
      it('Should get all parsers for yaml-js obj for compose v2 yaml', () => {
        ...
        const parsers = CFComposeModel.getParsersForYaml(yaml);
        expect(parsers).to.have.all.keys(['Parser', 'ServiceParser', 'NetworkParser', 'VolumeParser']);
    ...
    describe.only('Change default promise', () => {
      it('', () => {

        const config = require('./../Config');
        const Default = config.getDfaultPromise()
        CFComposeModel.setDefaultPromise(require('q').defer());
        expect(Default).to.be.equal(require('q').defer());
      });
